...
def _handle_connection(conn: socket.socket, addr, hub: Hub, recorder: BagRecorder) -> None:
        ...
        with conn:
            while True:
                ...
                if msg_type == MSG_FRAME:
                    ...
                    recorder.write_frame(ts_us, jpeg)
...
def serve_tcp_ingest(host: str, port: int, hub: Hub, recorder: BagRecorder,
    ...
    firmware) doesn't need any special-casing here.
    """
    ...
    try:
        while not stop.is_set():
            try:
                ...
                continue
            threading.Thread(target=_handle_connection, args=(conn, addr, hub, recorder),
                             daemon=True).start()
    finally:
        srv.close()
